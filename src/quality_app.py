...
class QualityApp(tk.Tk):
    ...
    def _build_log_tab(self):
        ...
        ttk.Button(act, text="🛑 Toggle Quarantine",
    ...
    def _submit_defect(self):
        ...
        try:
            ...
            # Automatically quarantine the box upon reporting any defect
            is_quar = 1
            ...
            # Automatically update the records table to 'Quarantined' to remove it from 'In Rack' or 'Consumed'
            c.execute("UPDATE records SET status='Quarantined' WHERE sub_batch_id=?", (rec["sub_batch_id"],))
    ...
    def _prompt_capa(self, row_id):
        ...
        tk.Label(win, text="Root Cause (السبب الجذري):", bg=BG_COLOR, fg=TEXT_COLOR, font=HMI_FONT_M).pack(anchor="w", padx=20, pady=(20, 5))
        ...
        tk.Label(win, text="Corrective Action (الإجراء التصحيحي):", bg=BG_COLOR, fg=TEXT_COLOR, font=HMI_FONT_M).pack(anchor="w", padx=20, pady=(15, 5))
    ...
    def _toggle_quarantine(self):
        ...
        try:
            ...
            sb_row = c.fetchone()
            if sb_row:
                new_status = 'Quarantined' if is_quarantined == 1 else 'In Rack'
                c.execute("UPDATE records SET status=? WHERE sub_batch_id=?", (new_status, sb_row[0]))
    ...
    def _open_excel(self):
        if os.path.exists(EXCEL_FILE):
            ...
            messagebox.showinfo("No File",
                                "No Excel file yet. Submit a defect first.")
